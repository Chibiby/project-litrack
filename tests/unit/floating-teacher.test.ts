 ...
 * is that the profile schema's `noAdvisorySection` is a declared choice that
 * lifts a requirement and is never written down. A stored flag could disagree
...
describe("teacherProfileSchema — the floating declaration", () => {
  ...
  it("defaults to false, so an old client cannot float a teacher by omission", () => {
    ...
    expect(parsed.data.noAdvisorySection).toBe(false);
  });
  ...
  it("is not persisted — the schema carries it, TeacherProfile does not", () => {
    ...
    expect(parsed.data).toHaveProperty("noAdvisorySection");
