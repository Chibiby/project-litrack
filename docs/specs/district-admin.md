...
| `src/lib/actions/accounts.ts` `impersonateUser` | Replace the `role === "SUPER_ADMIN"` refusal with an allow-list (`TEACHER`, `SCHOOL_HEAD`). As written today, a `DISTRICT_ADMIN` target would pass the refusal and the admin would be redirected to `/school-head`. |
...
| `src/components/admin/account-row-actions.tsx` | `DISTRICT_ADMIN` rows: no "Sign in as"; "Issue password" calls `resetDistrictAdminPassword`; show the assigned districts. |
...
| I14 | District admins cannot impersonate anyone and cannot be impersonated | `impersonateUser` target allow-list, plus `requireUser("SUPER_ADMIN")` on the caller + T13 |
...
| T13 | `tests/unit/actions/accounts.test.ts` (extend) | `impersonateUser` with a DA target gives NOT_FOUND. `resetDistrictAdminPassword` as a DA is forbidden. As an SA it sets `mustChangePassword: true`, and the audit metadata does not contain the credential. |
