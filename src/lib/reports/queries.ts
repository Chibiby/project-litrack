...
import { loadReportFrame, type GradeSectionLine } from "@/lib/reports/sheet-header";
...
/**
 * The template's "Reporting Period" for a report whose period is just the
 * hub's own date filters. Unset (the template then prints "As of <today>")
 * when neither edge was given. The school name and the generated-by line are
 * no longer summary lines: the template's header and footer carry both.
 */
function periodFromFilters(filters: ReportFilters): string | undefined {
  if (!filters.from && !filters.to) return undefined;
  return `${filters.from ?? "start"} to ${filters.to ?? "today"}`;
}

/** `resolveDateRange`'s label without its "Range: " prefix — the info row already says "Reporting Period". */
function periodFromRange(label: string): string {
...
/** Grade and section from the first learner in an already-filtered roster, else "All Classes" / a grade-only line. */
function gradeSectionLabelFromLearners(
  ...
  if (filters.gradeLevelId && first) {
    return { gradeLevel: grade, section: null };
  }
...
export async function buildAttendanceTable(
  ...
  const gradeSection = gradeSectionLabelFromLearners(learners, filters);
  ...
  return {
    ...
    reportingPeriod: periodFromRange(range.label),
    gradeSection,
    summary: [
...
export async function buildTermGradesTable(
  ...
  return {
    ...
    reportingPeriod: [
      filters.term
        ? (TERM_PERIOD_LABELS[filters.term as keyof typeof TERM_PERIOD_LABELS] ?? filters.term)
        : "All terms",
      periodFromFilters(filters),
    ]
      .filter(Boolean)
      .join(", "),
    gradeSection,
    summary: [`${groups.size} learner-term row(s)`],
    frame,
    columns: [
      { header: "Learner", width: 26 },
      { header: "Grade", width: 11 },
...
export async function buildClassRosterTable(
  ...
  return {
    title: "Class Roster",
    reportingPeriod: periodFromFilters(filters),
    gradeSection,
    summary: [`${learners.length} learner(s)`],
    frame,
    columns: [
      { header: "#", width: 6 },
...
export async function buildTeacherSummaryTable(
  ...
  const gradeSection = ((): GradeSectionLine => {
    const grade = firstSection
      ? (GRADE_LEVEL_LABELS[firstSection.gradeLevel.type] ?? firstSection.gradeLevel.type)
      : null;
    if (filters.sectionId && firstSection) return { gradeLevel: grade, section: firstSection.name };
    if (filters.gradeLevelId && firstSection) return { gradeLevel: grade, section: null };
    return { label: "All Classes" };
  })();
  const frame = await loadReportFrame({
    schoolName: scope.schoolName,
  ...
  return {
    ...
    reportingPeriod: periodFromFilters(filters),
    gradeSection,
    summary: [`${sections.length} class(es)`],
    frame,
    columns: [
...
export async function buildMosyTable(
  ...
  return {
    title: "MOSY Report",
    reportingPeriod: `MOSY window: ${window.label}`,
    gradeSection,
    summary: [
      schoolYear
