 ...
 * run — refusing is always better than proceeding with no way back.
 *
...
/** Typed into the confirmation box before a destructive action will run. */
...
export async function resetOperationalData(formData: FormData): Promise<ActionResult<{ removed: SnapshotCounts }>> {
  ...
  try {
    const safety = await takeSafetySnapshot("clear operational data");
    if (!safety.ok) return safety;
    ...
    await writeAudit({
      ...
      resource: "Database",
      metadata: { rowsRemoved: totalOf(removed), safetyStamp: safety.data?.stamp },
    });
...
export async function resetAllSchoolAccounts(formData: FormData): Promise<ActionResult<{ processed: number; failed: number }>> {
  ...
  try {
    const safety = await takeSafetySnapshot("reset school accounts");
    if (!safety.ok) return safety;
...
export async function removeAllTeachers(formData: FormData): Promise<ActionResult<{ processed: number; failed: number }>> {
  ...
  try {
    const safety = await takeSafetySnapshot("remove teacher accounts");
    if (!safety.ok) return safety;
