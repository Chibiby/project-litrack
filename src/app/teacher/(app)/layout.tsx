import { redirect } from "next/navigation";
import { requireUser } from "@/lib/auth/session";
import { getSchoolName } from "@/lib/cache/school";
import { getTeacherShellGrades } from "@/lib/dashboard/aggregates";
import { getTeacherOverview } from "@/lib/dashboard/teacher-overview";
import { buildTeacherNotifications } from "@/lib/dashboard/teacher-notifications";
import { GRADE_LEVEL_LABELS } from "@/lib/constants/enum-labels";
import { RoleShell } from "@/components/role-shell";
import type { ShellNotification } from "@/components/shell/notifications-menu";
import {
  aralAttendanceHref,
  aralReadingHref,
  gradeProfilingHref,
} from "@/components/dashboard/teacher/hrefs";
import { PostLoginSplash } from "@/components/post-login-splash";

export const dynamic = "force-dynamic";

export default async function TeacherAppLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  const user = await requireUser("TEACHER");

  // Only gate real teachers — SUPER_ADMIN may view teacher pages without profiling.
  if (user.role === "TEACHER" && !user.profileCompleted) {
    redirect("/teacher/profiling");
  }

  const userName = user.fullName || `${user.firstName} ${user.lastName}`;

  let schoolName: string | undefined;
  let grades: { id: string; label: string; hasAral?: boolean }[] | undefined;
  let notifications: ShellNotification[] | undefined;

  // Layouts cannot read searchParams; super-admin school impersonation still
  // gets admin nav via role === SUPER_ADMIN. Real teachers get grade links.
  // Shell chrome must not throw on transient pool errors — that tears down
  // RoleShell for the whole /teacher tree. Degrade to a minimal sidebar.
  if (user.role === "TEACHER" && user.schoolId) {
    const scope = {
      schoolId: user.schoolId,
      teacherId: user.id,
      isSuperAdmin: false,
    };

    try {
      // The triage read is the same cached entry the dashboard renders from, so
      // the bell and the dashboard's lanes can never report different numbers,
      // and on /teacher itself this costs nothing beyond the first fetch.
      const [name, shellGrades, overview] = await Promise.all([
        getSchoolName(user.schoolId),
        getTeacherShellGrades(scope),
        getTeacherOverview(scope),
      ]);

      schoolName = name ?? undefined;
      grades = shellGrades.map((g) => ({
        id: g.id,
        label: GRADE_LEVEL_LABELS[g.type],
        hasAral: g.hasAral,
      }));

      notifications = buildTeacherNotifications({
        pendingAralProfiling: overview.pendingAralProfiles,
        attendanceMissingThisWeek: overview.attendance.noClass,
        readingPending: overview.reading.pending,
        aralHref: gradeProfilingHref(overview.primaryAralGradeId),
        attendanceHref: aralAttendanceHref(overview.primaryAralGradeId),
        readingHref: aralReadingHref(overview.primaryAralGradeId),
      });
    } catch (err) {
      console.error("[teacher/layout] shell grades/school name failed:", err);
    }
  }

  return (
    <>
      {/* Sibling of RoleShell (also portaled to body) so chrome cannot contain it. */}
      <PostLoginSplash role="teacher" />
      <RoleShell
        role={user.role}
        userName={userName}
        schoolName={schoolName}
        grades={grades}
        notifications={notifications}
      >
        {children}
      </RoleShell>
    </>
  );
}
