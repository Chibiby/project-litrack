...
import { AralAttendancePanel } from "@/components/aral/aral-attendance-panel";
...
export default async function AralGradeAttendancePage({
  params,
  searchParams,
}: PageProps) {
  ...
  const [gradeSections, learners, attendances, dayMeta, shellGrades] =
    await Promise.all([
      ...
      prisma.attendanceDayMeta.findUnique({
        where: {
          gradeLevelId_date: { gradeLevelId: grade.id, date: selectedDate },
        },
  ...
  return (
    <AppShell
      title={`ARAL Attendance — ${GRADE_LEVEL_LABELS[grade.type]}`}
      subtitle={`${selectedDate.toLocaleDateString()}${isSuperAdmin && sp.schoolId ? " (Admin View)" : ""}`}
      role={user.role}
      ...
      <Card>
        <Suspense fallback={<TableSectionSkeleton rows={8} columns={5} />}>
          <AralAttendancePanel
            ...
            initialExisting={attendances}
            initialIsHoliday={dayMeta?.isHoliday ?? false}
